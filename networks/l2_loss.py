    ...
    def forward(self,
        ...
        if self.learn_log_variance or self.normlized_weight:
            ...
            if self.learn_log_variance:
                ...
                # [关键修正] 对 logvar 进行截断，防止随训练被优化至严重负数。
                # 严重的负数会导致 exp(logvar) 极度接近 0，使得误差项瞬间放大成百上千倍，引发不可逆的梯度爆炸。
                clamped_logvar = torch.clamp(self.logvar, min=-4.0, max=10.0)
                loss = loss / (torch.exp(clamped_logvar)) + clamped_logvar
