...
class MoeMLP(nn.Module):
    #将pretraining_tp值1是单次矩阵运算，2是对齐分布式多卡运算的精度
...
class ChannelMoeBlock(nn.Module):
    ...
    #【新增】定义一个类变量，所有实例共享，用于控制全局只打印一次!!!!!!!!!!!!!!!!
    _has_logged_once = False
    ...
    def forward(self, hidden_states, posembed):
        #hidden_states形状为 [Batch, N, Embed_Dim]
        identity = hidden_states
        posembed = self.PosEmbedder(posembed)
        posembed = posembed.softmax(-1)

        # ======================= 修改后的调试代码开始 =======================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # 检查类变量，确保在整个程序运行期间只执行一次
        if not ChannelMoeBlock._has_logged_once:
            import logging
            # 只打印 Shape 和 Dtype，去掉了具体的权重数值打印
            logging.info(f"[Posembed Debug] Shape: {posembed.shape} | Dtype: {posembed.dtype}")
            
            # 将标志位设为 True，后续任何 Layer 的任何 Step 都不会再打印
            ChannelMoeBlock._has_logged_once = True
        # ======================= 修改后的调试代码结束 =======================
        ...
        for i, expert in enumerate(self.experts):
            ...
            # 通道选择：在 768 个嵌入维度中，选出 Top-K 个最相关的“通道”
