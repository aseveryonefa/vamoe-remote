...
class VAMoE(nn.Module):
    ...
    def forward(self, x, target=None, posembed=None, run_mode='train', expert_weight=0.01):
        ...
        # ================= 增加查看x格式代码 =================！！！！！！！！！！！！！！！！！！
        # 使用 hasattr 检查标记，确保只在第一次运行时打印
        if not hasattr(self, 'has_logged_input_shape'):
            # 仅在主进程(rank 0)或非分布式环境下打印，避免多卡训练重复刷屏
            if not dist.is_initialized() or dist.get_rank() == 0:
                logging.info(f"\n{'='*20} [VAMoE Input Check] {'='*20}")
                logging.info(f"Input 'x' shape after encoder: {x.shape}")
                logging.info(f"{'='*60}\n")
            # 设置标记，防止后续 Batch 重复打印
            self.has_logged_input_shape = True
        # ================= [新增代码结束] =================
        ...
        # self.blocks的数量由encoder_depths决定，修改启用梯度检查点（源码上方）!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        for i, blk in enumerate(self.blocks):
            if self.model_type == 'vit':
                if self.training:
